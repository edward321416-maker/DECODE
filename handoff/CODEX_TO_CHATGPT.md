...
- `foundation/` (`npm ci`, Node v24.14.0): clean install. Initial `npm ci` with `tsx@4.19.2` reported 2 moderate `npm audit` findings (esbuild dev-server request forwarding, GHSA-67mh-4wv8-2f99, a transitive `tsx` devDependency, never exploitable here since no dev server is run — `tsx` is used only to execute `node --test` files); bumped `tsx` to `4.23.13` in a follow-up commit, which pulls in patched `esbuild`. Fresh `npm ci` afterward: **0 vulnerabilities.**
- `npm run typecheck` (`tsc`, strict, `noUncheckedIndexedAccess`): **0 errors**, both before and after the `tsx` bump.
- `npm test` (`node --test` over `foundation/test/*.test.ts` via `tsx`, no external test-framework dependency): **53/53 PASS, 0 failures**, both before and after the `tsx` bump. Each test is annotated with the PLAN 1A Section 12 acceptance-invariant number(s) it covers (invariants 2–59 that are in PR-A scope; invariants 1, 60–62 are runtime/repository-level facts verified separately below, not `foundation/` unit behavior).
- `node scripts/check-operating-docs.mjs` (default, working tree, final HEAD): **893/893 PASS, 0 failures.**
...
- `node scripts/check-operating-docs.mjs --tracked` (final committed HEAD `a5a448be4e6075f38c286118e45d088bad933055`): **964/964 PASS, 0 failures.**
- `git diff APPROVED_IMPLEMENTATION_BASE_SHA..HEAD --check`: exit 0, no whitespace errors. Exactly 27 files changed (verified by `git diff --name-status`), matching the FILES CHANGED list below.
