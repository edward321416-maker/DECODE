...
`APPROVED_IMPLEMENTATION_BASE_SHA = PENDING_POST_RECEIPT_MERGE` inside this receipt PR — the canonical PR-A base is fixed only once this receipt PR itself is reviewed and merged, as the then-current main HEAD. Only after that may Engineering start PLAN 1A PR-A Canonical Foundation using TDD. Material unresolved decisions must return through the Mandatory Decision Interview Gate. No automatic merge. ACTUAL TEST remains NOT YET TESTED.
