...
[Schema 0.1.0-candidate](../data/schemas/annotation-0.1.0-candidate.schema.json), [semantic validation](../annotation/src/validation.mjs) and [SIMULATED samples](../data/samples/README.md) implement the approved infrastructure proposal. Shape validation alone is insufficient: semantic checks enforce consent, sampling, alternatives, timing and review history. [Design](ANNOTATION_IMPLEMENTATION.md) states provisional nullability/timing choices; none is LOCKED. Operator metadata preserves unknown player/map/agent/rank values as null. ANNOTATION_RESEARCH consent scope does not authorize publication/training reuse or prove consent. Difficulty/adjudication/aggregate research reports remain outside this minimal tool and require a reviewed real-run protocol. See [EXPERIMENT_PROTOCOL](EXPERIMENT_PROTOCOL.md) for evidence labels and execution gates.
