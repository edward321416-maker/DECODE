...
[data/schemas](../data/schemas/README.md) is reserved for versioned machine-readable schemas. [data/samples](../data/samples/README.md) is reserved for conspicuously SIMULATED fixtures. No JSON schema or records are created by this documentation setup; machine schemas may be implemented only under the current approved engineering handoff and applicable foundation gate — this Team OS task does not implement the schema. Execution/ACTUAL authority for the ten-case design is the canonical [10-Case ACTUAL TEST Protocol v1.0](superpowers/specs/2026-09-06-decode-10-case-actual-test-protocol-v1.md) (Q1–Q56); [EXPERIMENT_PROTOCOL](EXPERIMENT_PROTOCOL.md) is retained only as a historical candidate summary, superseded for execution. Eight expert fields, context split and twelve seed principles above remain LOCK CANDIDATE regardless of which protocol document is read.
