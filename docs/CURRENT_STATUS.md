...
- **Canonical `main` is unaffected by this work until PR #17 merges.** The exact canonical `origin/main` HEAD remains `d4bc7b8018398dc9644088cda64bc17f6eb63021` (PR-A Canonical Foundation MERGED via PR #15, canonical post-merge receipt merged via PR #16). Nothing in this section — the D025–D037 decisions, the design spec, or this status text itself — is canonical policy until the PR that carries it is reviewed and merged. This document's own content, while read on an unmerged branch, describes a **reviewed proposal**, not yet current main state, exactly like every prior not-yet-merged revision in this repository's history.
- **PR #17 (`claude/dry-readiness-design`) is OPEN / NOT MERGED.** It is a spec-review branch: Product's independent review of its prior head `966ad18893cb0a8450ad3f63ed8a04c08a52740e` returned verdict `REVISE — MERGE NOT AUTHORIZED`. This revision is the correction round for that verdict; the corrected head still awaits a fresh Product review before any merge is authorized. Do not read D025–D037 or the design spec as canonical main policy merely because they exist in this working tree — they become canonical only when PR #17 is actually merged.
